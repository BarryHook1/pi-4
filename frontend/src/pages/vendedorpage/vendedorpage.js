...
const VendedorPage = () => {
    ...
    const [activeTab, setActiveTab] = useState('produtos'); // 'produtos' ou 'propostas'
    ...
    const fetchProposals = async () => {
        const vendedorId = localStorage.getItem('vendedorId');
        try {
            const response = await fetch(`http://localhost:8080/vendedor/proposals/${vendedorId}`);
            const data = await response.json();
            setProposals(data);
        } catch (error) {
                    ...
                    {proposals.length > 0 ? (
                        proposals.map((proposal) => (
                            <div key={proposal._id} className="proposal-card">
                                ...
                                <p><strong>Data:</strong> {new Date(proposal.date).toLocaleString()}</p>
                            </div>
                        ))
                    ) : (
